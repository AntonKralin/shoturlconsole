"""main module. start application"""
from utils.storage import StorageService


def main():
    """main function. it start application"""
    ...
    while True:
        ...
        if choice == 1:
            ...
            storage_service.append(http)
        elif choice == 2:
            alias = input('Введите псевдоним домашней страницы: ').lower()
            storage_service.print_by_alias(alias)
        elif choice == 3:
            shot_url = input('Введите короткий интернет-адресс: ').lower()
            storage_service.print_by_short(shot_url)
        elif choice == 4:
            storage_service.print_all()
        else:
            break
